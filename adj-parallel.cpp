...
int main(int argc, char *argv[]){
	...
	int p ;
	 MPI_Comm_size(MPI_COMM_WORLD, &p);
        printf(" Order\tDegree\t Chunk_size\tThreads\tTime_Serial\tTime_OpenMP\tTime_MPI\tDist\t Diam\tProcesses\n");
        printf("%d\t%d\t%d\t%d\t%f\t%f\t%f\t%f\t%d\t%d\n",order,degree,CHUNK_SIZE,nT,time_Serial,time_OpenMP,time_MPI,g_dist,g_diam,p); 
    }
    MPI_Finalize();
    return 0; 
}
